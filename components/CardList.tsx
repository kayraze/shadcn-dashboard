...
const CardList = ({ title }: { title: "Popular Content" | "Latest Transactions" }) => {
  const items = title === "Popular Content" ? popularContent : latestTransactions
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-4">
        {items.map(item => (
          <Card key={item.id} className="flex-row items-center justify-between p-4 gap-0">
            <div className="flex gap-4 w-full">
              <div className="w-12 h-12 rounded-sm relative overflow-hidden">
                <Image src={item.image} alt={item.title} fill />
              </div>
              <CardContent className="p-0">
                <CardTitle className="text-xs sm:text-[16px] font-medium">{item.title}</CardTitle>
